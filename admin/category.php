        ...
        <div class="row">
            ...
            <div class="col-md-10 py-5 px-5 user" style="min-height: 100vh;">
                <div class="container px-5 py-5" style="border-radius: 10px;background-color: rgb(197, 162, 98);">
                    <a href="" data-bs-toggle="modal" data-bs-target="#addcate" style="color: blue"><i class="fa-solid fa-circle-plus"></i> Thêm danh mục</a>
                ...
                <?php
                    ...
                    while($row = mysqli_fetch_array($query)){
                        $cateid = $row['cat_id'];
                        $name = $row['cat_title'];
                        $products = "SELECT COUNT(*) AS count_products FROM products WHERE product_cat = '$cateid'";
                        $query_products = mysqli_query($con,$products);
                        $row_count = mysqli_fetch_array($query_products);
                        echo '
                            <form action="change_cate.php" method="POST">
                            <div class="row mb-2 cart-top">
                                <div class="col-md-2">
                                    <input class="form-control" type = "text" name="cateid" value="'.$cateid.'" readonly>
                                </div>
                                <div class="col-md-3">
                                    <input type="text" name="catename" value="'.$name.'" class="form-control">
                                </div>
                                <div class="col-md-2">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <a cateid='.$cateid.' class="btn btn-danger delete-cate"><i class="fa-solid fa-trash-can"></i></a>
                                </div>
                            </div>
                        </form>
                        ';
    ...
    <div class="modal fade" id="addcate" tabindex="-1" aria-labelledby="addcatelabel" aria-hidden="true">
        <div class="modal-dialog">
            ...
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="addcatelabel">Thêm danh mục</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
            <div class="container-fluid">
                <form id = "addcate-form">
                    <div class="mb-3">
                        <label for="catename" = class="form-label">Tên danh mục</label>
                        <input type="text" name="catename" class="form-control">
                    </div>
                    <button type="submit" id = "addcate-btn" class="btn btn-primary">Thêm</button>
                </form>
            </div>
            </div>
